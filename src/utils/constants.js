/** @format */

//export const API_URL = 'http://192.168.3.35:8080/apipedidosprofit'
//export const API_URL = 'http://192.168.0.106/apipedidosprofit'
//export const API_URL = 'http://192.168.10.119:8080/apipedidosprofit'
export const API_URL = 'http://grupodh.dyndns.org:8080/apiPedidosProfit'
//export const API_URL = 'https://react-native-ecommerce-dev.herokuapp.com'
export const TOKEN = 'token'
export const SEARCH_HISTORY = 'search-history'
export const SEARCH_HISTORY_CLIENT = 'search-history-client'
export const CART = 'cart'
export const ARTICULOS = 'art'
export const CLIENT = 'client'
export const CLIENTES = 'clientes'
export const SUCURSAL = '0001'
export const ALMACEN = '001'
export const DB = 'DHHDIV_A'
export const TASA = 'tasa'
export const PEDIDOS = 'pedidos'
export const TRANSPORTE = 'transporte'
export const FORMA_PAGO = 'forma_pago'
export const FECHA_VENCIMIENTO = new Date(2021, 12, 31)
export const FECHA_ACTUAL = new Date()
export const STRAPI_PUBLISHABLE_KEY =
	'pk_test_51IGQJXFotiOk2WnmTOM6HpcPnfjZSX98o9FqA5RJiMKB6jm62hiVu7mtk9rxgMjPSbjdIJxGmN6jwFNQ69IQ7f9O00AthydrqM'
